...
#define MIN_POWER 600   // Set to 2000 for full range
#define MAX_POWER 2100  // Set to 2000 for full range
...
public:
  ...
  Motor(int i)
    : index(i) {}
  void update() {
    int us = constrain((int)(Power + OFFSET[index]), MIN_POWER, MAX_POWER);
...
void PID_X() {
  ...
  m[0].Final = constrain(m[0].Final, MIN_POWER, MAX_POWER);
  m[2].Final = constrain(m[2].Final, MIN_POWER, MAX_POWER);
...
void PID_Y() {
  ...
  m[1].Final = constrain(m[1].Final, MIN_POWER, MAX_POWER);
  m[3].Final = constrain(m[3].Final, MIN_POWER, MAX_POWER);
...
void PID_Z() {
  ...
  for (int i = 0; i < 4; i++) {
    m[i].Final = constrain(m[i].Final, MIN_POWER, MAX_POWER);
  }
